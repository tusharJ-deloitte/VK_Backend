...
class MysteryRoomCollection(models.Model):
    ...
    number_of_team_members = models.IntegerField(default=0, null=True, blank=True)
...
class MysteryRoom(models.Model):
    mystery_room = models.ForeignKey(MysteryRoomCollection, on_delete=models.CASCADE)
    banner_image = models.TextField(null=True, blank=True)
    title = models.TextField(unique=True, null=True, blank=True)
    EASY = "EASY"
    MEDIUM = "MEDIUM"
    ...
    difficulty_level = models.CharField(max_length=20, choices=DIFFICULTY, default=EASY)
...
class MysteryRoomQuestion(models.Model):
    ...
    mystery_room_collection = models.ForeignKey(MysteryRoomCollection, on_delete=models.CASCADE)
    ...
    question_type = models.CharField(max_length=20, choices=QUESTION_CHOICES, default=MCQ)
    created_on = models.TextField(null=True, blank=True)
    last_modified = models.TextField(null=True, blank=True)
