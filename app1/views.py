...
from notifications_app.utils import schedule_notification_task
...
def create_event(request):
    if request.method == 'POST':
        try:
            ...
            # schedule notification for creation of event, (1hr,10mins,5mins) before event start
            time_format = "%Y-%m-%d %H:%M:%S"
            time_data = python_data["startDate"]+" "+python_data['startTime']
            start_datetime = datetime.strptime(time_data,time_format)
            schedule_notification_task(python_data['name'],start_datetime)
