...
try:
    ws = create_connection("wss://8u1oqx7th4.execute-api.us-east-1.amazonaws.com/production?email=backend@gmail.com")
    ...
    try:
        if request.method == 'POST':
            ...
            if "email" in python_data and "name" in python_data:
                ...
                response = sns(python_data["email"],"Subscribe to AWS SNS Service",msg)
                if response :
                    print("email sent")
                else:
...
def update_teams(request, team_id):
    try:
        if request.method == 'PUT':
            ...
            activity_id = activity_instance.pk
            print(activity_instance)
            team_activity_instance = Team.activity.through.objects.filter(team_id=team_id)
            ...
            for key, value in d.items():
                ...
                if value == 1:
                    ...
                    print("creating new entry in notifications table for member removed from team")
...
def register_individual(request):
    try:
        ...
        # print(python_data)
        eventInstance = Event.objects.filter(id=python_data['event_id'])
        userInstance = User.objects.filter(email=user['user_email'])
    ...
    try:
        ...
        for i, event in enumerate(events):
            if event.event_type == "Group":
                ...
                print("before else::", players, not_participated)
            else:
                    ...
                    for team in all_teams:
                        ...
                        for player in players:
                            ...
                            if plr.user_id == user.pk:
                                ...
                                event.save()
                                msg = f"Hi {user.first_name}, Your registration for event {event.name} is cancelled."
                                response = sns(p_id,"Cancelled registration",msg)
                                if response :
                                    print("email sent")
                                else:
                                    print("not subscribed to email service")
...
def upload_aws(request):
    try:
        if request.method == 'POST':
            ...
            if len(event) == 0:
                raise Exception(json.dumps({"message": "event not found", "status": 400}))
            event=event[0]
            ...
            for item in upload:
                ...
                if item.file_name[0:10]==str(datetime.datetime.now()).split(' ')[0]:
                    raise Exception(json.dumps({"message": "upload already exists", "status": 400}))
            ...
            upload_instance.file_name = str(upload_instance.uploaded_on).split(' ')[0]+python_data['event_name']+python_data["user_email"].split('@')[0]+".mp4"
            print("done---1")              
        ...
        if request.method=="POST":
            ...
            for upload in upload_instance:
                if str(upload.uploaded_on).split(' ')[0] == python_data['date']:
                    upload.score = python_data['score']
                    upload.save()
            ...
            for i in ind:
                if i.player.user.email == python_data['user_email']:
                    ...
                    player.score = player.score + python_data['score']
                    player.save()
...
def get_plank_results_of_event(request,event_id):
    ...
    try:
        ...
        uploads = Upload.objects.filter(event=event).values("user").annotate(total_score=Sum('score')).order_by("-total_score")
        print(uploads)
        result=[]
...
def publish_quiz_for_event(request):
    try:
        ...
        if quiz.event_id !=0:
            raise Exception(json.dumps({"message": "quiz already published", "status": 400}))
